...
## 1. ¿Por qué Clases en el Backend?
...
## 2. Las 3 Capas Sagradas (Controller - Service - Repository)

Esta es la arquitectura estándar en la industria (NestJS, Java Spring, C# .NET).

### 2.1 Router (El Portero)
Solo define URLs y apunta al Controller. No tiene lógica.
```javascript
router.post('/users', userController.create);
```

### 2.2 Controller (El Recepcionista)
*   **Responsabilidad:** Hablar HTTP.
*   **Recibe:** `req` y `res`.
*   **Valida:** Body y Params.
*   **Llama:** Al Service.
*   **Responde:** JSON o Error.
*   **Regla:** NUNCA accedas a la base de datos aquí.
...
class UserController {
    ...
    async create(req, res) {
        const { email, password } = req.body; // validación básica
...
### 2.3 Service (El Cerebro)
*   **Responsabilidad:** Lógica de Negocio.
*   **No sabe:** Que existe HTTP (`req`, `res` prohibidos).
*   **Hace:** Hashing de contraseñas, enviar emails, cálculos complejos.
...
### 2.4 Repository (El Bibliotecario)
*   **Responsabilidad:** Hablar SQL / NoSQL.
*   **No sabe:** Nada de lógica de negocio. Solo `SELECT`, `INSERT`.
...
## 4. Inyección de Dependencias (El Contenedor)
...
Un solo archivo donde se conectan todos los cables.
...
app.get('/users', userController.create.bind(userController));
```
Ahora `UserService` recibe mágicamente el `UserRepository` sin que tú se lo pases.

---

## 5. Principios SOLID en Backend
...
## 6. Decoradores (Metaprogramación)
...
## 7. Factory Pattern (Creación Compleja)
...
## 8. Adapter Pattern (Estandarización)
...
## 10. Observer Pattern (Desacoplar Lógica)
...
## 11. Strategy Pattern (Polimorfismo en Runtime)
...
## 12. Command Pattern (Acciones como Objetos)
...
## 13. Active Record vs Data Mapper

¿Cómo hablan tus objetos con la BD?

### 13.1 Active Record (Mongoose, Sequelize v5)
...
### 13.2 Data Mapper (TypeORM, MikroORM)
...
## 14. Value Objects (Objetos de Valor)
...
## 15. Testing de Arquitectura (Mocking)
...
## 16. Singleton Pattern (Única Instancia)
...
## 17. Mixins (Composición sobre Herencia)
...
## 18. Template Method (Esqueleto de Algoritmo)
...
## 19. Patrones de Microservicios (Strangler Fig)
...
## 20. Prototype Pattern (Clonación)
...
## 23. Bridge Pattern (Desacoplar Abstracción)
...
## 24. Arquitectura Hexagonal (Ports & Adapters)
