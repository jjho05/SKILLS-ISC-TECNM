...
## 1. ¿Por qué necesitamos Frameworks?

Imagina construir un rascacielos con ladrillos y cemento (Vanilla JS) versus usar vigas de acero prefabricadas y grúas (Frameworks).
El "Spaghetti Code" es el enemigo.

### 1.1 El Problema del DOM
En JS puro, mantener el HTML sincronizado con los datos es difícil.
```javascript
// Variable JS
let count = 0;
// Elemento HTML
...
function incrementar() {
    count++;
    // Tienes que acordarte de actualizar el HTML manualmente
    span.textContent = count; 
}
```
Si tienes 50 variables y 200 vistas, esto es imposible de mantener.

### 1.2 La Solución: Reactividad
Los frameworks modernos hacen esto automáticamente:
*"Si cambia la variable `count`, actualiza el `span` tú solo. No me molestes."*

---

## 2. Evolución Histórica

### 2.1 La Era Oscura (1995-2005)
Todo era DHTML (Dynamic HTML). Scripts sueltos copiados de foros. Caos total.

### 2.2 La Era de jQuery (2006-2010)
jQuery estandarizó el acceso al DOM.
*   **Antes:** `document.getElementById('id')` (o peor en IE6).
*   **jQuery:** `$('#id')`.
*   **Filosofía:** "Write Less, Do More".
*   **Problema:** Sigue siendo imperativo ("Busca el botón, cambia su color"). No escala bien en apps gigantes.

### 2.3 La Era MVC (2010-2013)
AngularJS (v1), Backbone.js, Ember.js.
Intentaron traer patrones de Backend (Model-View-Controller) al Frontend.
*   **Two-Way Data Binding:** Si cambias el input, cambia la variable. Si cambias la variable, cambia el input.
*   **Problema:** Rendimiento pobre en móviles y complejidad.

### 2.4 La Era de los Componentes (2013-Hoy)
React cambió el juego.
*   **Filosofía:** "Todo es un componente". Un botón es un componente. El header es un componente. La página es un componente.
*   **Declarativo:** Tú dices *qué* quieres ("Quiero un botón azul"), no *cómo* hacerlo ("Busca el div, añádele la clase...").

---

## 3. El "Big Three" (y los Retadores)

No existe el "mejor framework". Existe la herramienta correcta para el trabajo.

### 3.1 React (Facebook/Meta)
*   **Estatus:** El Rey indiscutible (60-70% del mercado).
*   **Filosofía:** "Es solo una librería, no un framework". Solo maneja la Vista (V). Tú eliges el Router, el CSS, etc.
*   **Sintaxis:** JSX (HTML dentro de JS).
*   **Curva de aprendizaje:** Media/Alta (Entender el `useEffect` es difícil).

### 3.2 Vue.js (Evan You)
*   **Estatus:** El favorito de la comunidad. Muy popular en China y PHP (Laravel).
*   **Filosofía:** "Framework Progresivo". Empieza simple (CDN) y escala a una app gigante.
*   **Sintaxis:** HTML estándar con "directivas" mágicas (`v-if`, `v-for`).
*   **Curva de aprendizaje:** Baja (Muy fácil empezar).

### 3.3 Angular (Google)
*   **Estatus:** El estándar Enterprise (Bancos, Corporaciones).
*   **Filosofía:** "Baterías incluidas". Te obliga a usar TypeScript, RxJS, su propio Router, su propio HTTP Client.
*   **Sintaxis:** HTML + TypeScript (Clases). Estricto.
*   **Curva de aprendizaje:** Muy Alta (Muchos conceptos propios).

### 3.4 Los Modernos: Svelte y Solid
*   **No tienen Virtual DOM.** Compilan el código a JS puro y quirúrgico.
*   **Rendimiento:** Brutalmente rápidos.
*   **Svelte:** Escribes menos código que en ningún otro.

---

---

## 4. Virtual DOM vs Signals (La Guerra Técnica)

¿Cómo saben los frameworks qué parte de la pantalla actualizar?

### 4.1 Virtual DOM (React, Vue 2)
...
### 4.2 Signals (Solid, Svelte, Vue 3, Angular 16+)
El nuevo estándar.
...
## 5. Gestión del Estado (State Management)

El problema #1 en desarrollo frontend: *"¿Dónde guardo los datos?"*

### 5.1 Local State (Component level)
Datos que solo le importan a un botón (ej. `isOpen`, `isLoading`).
*   **React:** `useState`.
*   **Vue:** `ref`.

### 5.2 Prop Drilling (El Dolor)
Pasar datos del Abuelo -> Padre -> Hijo -> Nieto solo para que el Nieto lo use.
Es un anti-patrón.

### 5.3 Global State (Context / Store)
Un "banco de datos" accesible desde cualquier componente.
*   **React:** Context API, Redux (Viejo), Zustand (Nuevo estándar).
*   **Vue:** Pinia (Adiós Vuex).
...
Todo componente nace, vive y muere. Entender esto evita Memory Leaks.

### 6.1 Montaje (Mounting)
El componente aparece en pantalla por primera vez.
*   Uso: Hacer peticiones API, suscribirse a eventos globales.
*   **React:** `useEffect(() => {}, [])`.
*   **Vue:** `onMounted()`.

### 6.2 Actualización (Updating)
El usuario interactuó o llegaron datos nuevos. El componente se re-renderiza.
*   **React:** `useEffect(() => {}, [variable])`.
*   **Vue:** `watch()`.

### 6.3 Desmontaje (Unmounting)
El usuario cambió de página. El componente se borra.
*   **Crucial:** Aquí debes cancelar peticiones AJAX pendientes y limpiar `setIntervals`. Si no, tu app se vuelve lenta.
*   **React:** `return () => { limpieza }` dentro de `useEffect`.
*   **Vue:** `onUnmounted()`.
...
## 7. Routing (Navegación sin recarga)
...
## 8. Estilando Componentes (CSS en la era de JS)

¿Cómo evitamos que `.boton` afecte a toda la app?

### 8.1 CSS Modules (`Boton.module.css`)
...
### 8.2 CSS-in-JS (Styled Components / Emotion)
...
### 8.3 Utility-First (Tailwind CSS)
El rey actual.
...
## 9. Data Fetching Moderno (Olvida useEffect)
...
## 10. Build Tools: La Revolución de Vite

Nadie escribe `<script type="module">` a mano. Usamos bundlers.

### 10.1 Webpack (El Gigante Dormido)
...
## 11. Estrategias de Renderizado (Rendering Patterns)

No todo es SPA. Google necesita leer tu HTML.

### 11.1 CSR (Client Side Rendering)
...
### 11.3 SSG (Static Site Generation)
...
### 11.4 ISR (Incremental Static Regeneration)
...
## 13. El Ecosistema Moderno (Stack 2025)
...
## 14. Arquitectura de Carpetas Escalable

No pongas todos tus componentes en `src/components`.

### 14.1 Feature-Based Architecture
...
## 15. Optimización de Rendimiento (Performance)

React es rápido, pero tú puedes hacerlo lento si no tienes cuidado.

### 15.1 Re-renders Innecesarios
...
### 16.2 Almacenamiento de Tokens (JWT)
...
## 19. PWA (Progressive Web Apps)

Hacer que tu web se sienta como una App nativa.

### 19.1 Service Workers
...
### 19.2 Web App Manifest (`manifest.json`)
...
## 20. La Gran Matriz Comparativa (2025)
...
## 21. Patrones Enterprise

Cuando tu equipo crece de 5 a 50 desarrolladores.

### 21.1 Monorepos (Turborepo / Nx)
...
### 21.2 Micro-Frontends (Module Federation)
...
## 22. Formularios Avanzados (React Hook Form)

Manejar formularios en React puro es lento (cada letra renderiza todo).

### 22.1 Uncontrolled Components
...
## 25. El Futuro: React Compiler (React Forget)
...
## 26. Carrera y Recursos (Path to Senior)

¿Dónde aprender más?

### 26.1 Roles Especializados
