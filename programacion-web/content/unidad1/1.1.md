...
La **World Wide Web (WWW)**, comúnmente conocida como la Web, no es solo una red de computadoras; es el sistema de información más grande jamás construido por la humanidad. Desde su invención en 1989 por **Tim Berners-Lee** en el CERN (Organización Europea para la Investigación Nuclear), ha pasado de ser un simple repositorio de documentos estáticos a convertirse en el sistema operativo universal de nuestra era.

Entender su evolución es crucial para cualquier Ingeniero en Sistemas, ya que las tecnologías actuales no surgieron en un vacío. Son respuestas directas a las limitaciones y problemas de las eras anteriores. 
*   ¿Por qué usamos Cookies? Porque HTTP es "sin estado".
*   ¿Por qué existe AJAX? Porque recargar la página entera era ineficiente.
*   ¿Por qué usamos Contenedores? Porque configurar servidores físicos era lento y propenso a errores.

En esta unidad, diseccionaremos la historia de la web en **cuatro eras generacionales**, analizando no solo *qué* pasó, sino *por qué* pasó y *cómo* funciona técnicamente cada avance.

---

## 2. Web 1.0: La Era de la Información Estática (1990 - 2004)

### 2.1 Definición y Filosofía (The Read-Only Web)

La **Web 1.0** se refiere al estado de la World Wide Web entre su nacimiento y aproximadamente 2004. Durante este período, la gran mayoría de los usuarios eran **consumidores** de contenido. La creación de contenido estaba reservada para una élite técnica ("webmasters") que sabía escribir HTML a mano.

La filosofía subyacente era la **publicación**. La web se veía como una extensión digital de los medios impresos: periódicos, revistas y enciclopedias.

### 2.2 Contexto Histórico: Cronología de la Guerra de los Navegadores
...
### 2.3 Profundización: La Guerra de los Motores de Renderizado
...
### 2.4 Cronología Detallada de Versiones HTML
...
### 2.3 Tecnología Profunda: Cómo funcionaba la Web 1.0
...
#### B. El Protocolo HTTP/1.0 y la falta de Estado (Stateless)
La decisión de diseño más crítica de Tim Berners-Lee fue hacer HTTP "stateless".
*   **Filosofía:** Mantener una conexión abierta consume memoria RAM. En 1990, la RAM era carísima. Para escalar a millones de usuarios, el servidor debía olvidar al usuario inmediatamente para liberar recursos.
*   **Consecuencias Comerciales:**
    Esto impedía el comercio electrónico. ¿Cómo añades un producto al carrito si al hacer clic en "Seguir Comprando" el servidor ya olvidó quién eres y qué tenías en el carrito?
*   **La Curita (Workaround): Cookies (1994)**
    Lou Montulli de Netscape inventó las cookies. Un parche sobre HTTP.
    *   *Mecanismo:* El servidor envía `Set-Cookie: usuario=123`. El navegador guarda ese texto y lo envía de vuelta en **cada** petición futura `Cookie: usuario=123`.
...
### 2.4 Ejemplo de Código: Script CGI en Perl
...
## 3. Web 2.0: La Revolución Social y Colaborativa (2004 - 2010)

### 3.1 El Renacimiento tras la Burbuja
En el año 2000, la burbuja de las empresas "puntocom" estalló. NASDAQ cayó un 78%. Miles de empresas quebraron (Pets.com, Webvan).
Sin embargo, las que sobrevivieron (Amazon, eBay, Google) tenían algo en común: no solo vendían cosas, **conectaban personas**.

En 2004, **Tim O'Reilly** organizó la primera conferencia "Web 2.0". No era una nueva versión técnica (HTTP seguía siendo 1.1), sino un cambio de paradigma empresarial y de diseño.
...
### 3.3 La Revolución AJAX (Asynchronous JavaScript and XML)
Antes de 2005, si querías actualizar un dato en la página, tenías que recargarla completa.
*   **El Problema:** El "parpadeo" de la página rompía la concentración y la experiencia de usuario.
*   **La Solución:** El objeto `XMLHttpRequest` (creado por Microsoft para Outlook Web Access, irónicamente).

**Google Maps (2005)** fue la "Killer App" de AJAX. Demostró que podías arrastrar un mapa infinito descargando cuadritos de imágenes (tiles) en segundo plano sin recargar jamás la página.
...
### 3.4 Código: jQuery vs Vanilla JS (La Era Dorada de las Librerías)
...
### 3.5 El Auge de las APIs y SOAP vs REST
...
### 3.7 Bono Histórico: Las Guerras de los Navegadores
...
## 4. Web 3.0: Movilidad, Nube y Semántica (2010 - Actualidad)

### 4.1 La Tormenta Perfecta
Tres factores convergieron alrededor de 2010 para cambiar la web para siempre:
1.  **El Smartphone (iPhone 2007 + Android):** La web salió del escritorio.
2.  **La Nube (AWS 2006):** Infraestructura elástica y barata.
3.  **JavaScript V8 (Chrome 2008):** JS se volvió increíblemente rápido.

### 4.2 La Web Semántica (El sueño vs La realidad)
...
### 4.3 Single Page Applications (SPA)
La arquitectura de la web cambió radicalmente.
*   **Antes:** La lógica estaba en el servidor (PHP/JSP). El navegador solo pintaba HTML.
*   **Ahora:** Se descarga una aplicación completa en JavaScript (React/Angular/Vue) al inicio.
*   **Ventaja:** Sensación de aplicación nativa. Transiciones instantáneas.
*   **Desventaja:** SEO más complejo (los buscadores veían una página vacía), primera carga lenta.

Este cambio movió la complejidad del Backend al Frontend. Nació el puesto de "Frontend Engineer".

### 4.4 Profundización en SPAs: El Virtual DOM
...
## 5. Web 4.0: El Futuro Inteligente y Ubicuo

### 5.1 Definición: La Web Simbiótica
Si la Web 3.0 conectó datos, la Web 4.0 conecta **inteligencias**.
Se caracteriza por la integración profunda de la Inteligencia Artificial, el procesamiento de lenguaje natural y la inmersión.
...
#### C. Internet de las Cosas (IoT) y Edge Computing
La web se expande fuera de las pantallas.
*   Protocolos como **MQTT** y **CoAP** permiten que dispositivos con poca energía (focos inteligentes, sensores agrícolas) se comuniquen vía web.
*   **Edge Computing:** Procesar los datos en el borde de la red (cerca del usuario), no en un servidor central, para reducir la latencia a milisegundos (crítico para coches autónomos).

---

## 6. Evolución de los Protocolos: La Tubería de Internet
...
## 6. Diccionario de Cabeceras HTTP (Headers)

El verdadero "idioma" de la web no es HTML, son las cabeceras HTTP. Es la metainformación que negocian cliente y servidor antes de enviarse un solo byte de contenido.
...
## 9. Historia de la Seguridad Web: De Texto Plano a Zero Trust

La seguridad no fue una prioridad inicial. Se asumía que la red era académica y confiable.

### 9.1 La Era del Texto Plano (HTTP)
...
## 10. Evolución de las Herramientas de Desarrollo (DevTools)
...
## 12. Impacto Socioeconómico y Cultural de la Web
...
### 12.1 La Democratización del Conocimiento
...
### 12.2 La Economía Digital y el "Gig Economy"
...
### 12.3 Privacidad, Vigilancia y el "Capitalismo de Vigilancia"
...
## 13. Tendencias Futuras 2025-2030: Más allá de la Pantalla
...
### 13.1 Web 5.0 (La Visión de Jack Dorsey)
...
### 13.3 Interfaces Neuronales Directas (BCI)
